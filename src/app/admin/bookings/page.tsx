...
function BookingSheet({ booking, units, onClose, onAction }: { booking: AdminBooking; units: DressUnit[]; onClose: () => void; onAction: (id: string, body: Record<string, unknown>, success: string) => Promise<void> }) {
  ...
  return <Sheet open onOpenChange={(open) => { if (!open && !busy) onClose(); }}><SheetContent className="w-full overflow-y-auto bg-white sm:max-w-2xl" aria-describedby="booking-detail-description"><SheetHeader><SheetTitle className="font-serif text-xl">{booking.invoice_number}</SheetTitle><SheetDescription id="booking-detail-description">{booking.customer_name} · {booking.customer_whatsapp}</SheetDescription></SheetHeader><div className="space-y-6 px-4 pb-8">
    ...
    {['submitted', 'pending'].includes(booking.booking_status) ? <section className="space-y-3"><h3 className="font-serif font-bold">Konfirmasi booking</h3><div className="grid grid-cols-2 gap-3"><Field label="Mulai sewa"><input className={inputClass} type="date" value={rentalStart} onChange={(e) => setRentalStart(e.target.value)} /></Field><Field label="Selesai sewa"><input className={inputClass} type="date" value={rentalEnd} onChange={(e) => setRentalEnd(e.target.value)} /></Field><Field label="Biaya tambahan"><input className={inputClass} type="number" min="0" value={additionalFees} onChange={(e) => setAdditionalFees(Number(e.target.value))} /></Field><Field label="Total final"><input className={inputClass} type="number" min="0" value={totalAmount} onChange={(e) => setTotalAmount(Number(e.target.value))} /></Field><Field label="DP wajib"><input className={inputClass} type="number" min="0" value={depositRequired} onChange={(e) => setDepositRequired(Number(e.target.value))} /></Field><Field label="Batas pembayaran"><input className={inputClass} type="datetime-local" value={paymentDueAt} onChange={(e) => setPaymentDueAt(e.target.value)} /></Field></div>{needsUnit ? <div><span className="mb-2 block text-xs font-bold">Pilih unit busana</span><div className="max-h-40 space-y-2 overflow-y-auto rounded-xl border border-stone-200 p-3">{units.filter((unit) => unit.status === 'ready').map((unit) => <label key={unit.id} className="flex items-center gap-2 text-xs"><input type="checkbox" checked={unitIds.includes(unit.id)} onChange={(e) => setUnitIds((current) => e.target.checked ? [...current, unit.id] : current.filter((id) => id !== unit.id))} /><span><strong>{unit.unit_code}</strong> · {unit.dresses?.name || 'Busana'} · {unit.size} · {unit.color}</span></label>)}</div></div> : null}<button disabled={busy || !rentalStart || !rentalEnd || (needsUnit && !unitIds.length)} onClick={() => void run({ action: 'confirm', unitIds, rentalStart, rentalEnd, additionalFees, totalAmount, depositRequired, paymentDueAt: paymentDueAt ? new Date(paymentDueAt).toISOString() : undefined }, 'Booking dikonfirmasi dan instruksi pembayaran dibuka.')} className="w-full rounded-xl bg-emerald-600 px-4 py-3 text-sm font-bold text-white disabled:opacity-40"><CheckCircle2 className="mr-2 inline h-4 w-4" /> Konfirmasi & buka pembayaran</button><div className="flex gap-2"><input className={inputClass} value={declineReason} onChange={(e) => setDeclineReason(e.target.value)} placeholder="Alasan penolakan" /><button disabled={busy} onClick={() => void run({ action: 'decline', reason: declineReason }, 'Booking ditolak.')} className="shrink-0 rounded-xl bg-red-600 px-4 text-xs font-bold text-white"><XCircle className="mr-1 inline h-4 w-4" /> Tolak</button></div></section> : null}
