    ...
    <main className="mx-auto max-w-5xl space-y-6 px-4 py-10 sm:px-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
        ...
        <section className="space-y-4 rounded-3xl border border-stone-200 bg-white p-5 shadow-sm sm:p-7"><h2 className="font-serif text-xl font-bold">Pembayaran</h2>
          {booking.paymentDueAt ? <p className="rounded-xl bg-stone-50 p-3 text-xs text-stone-600">Batas pembayaran: <strong>{new Date(booking.paymentDueAt).toLocaleString('id-ID')}</strong></p> : null}
          ...
          {booking.paymentInstructions && canPay ? <form onSubmit={upload} className="space-y-4">
            ...
            {paymentMethod === 'qris' && booking.paymentInstructions.qris_enabled ? <div className="rounded-xl bg-stone-50 p-4 text-sm"><strong>Bayar melalui QRIS</strong><p className="mt-1 text-xs text-stone-500">Buka gambar QRIS, pindai melalui aplikasi pembayaran, lalu unggah buktinya di bawah.</p>{booking.paymentInstructions.qris_image ? <a href={booking.paymentInstructions.qris_image} target="_blank" rel="noreferrer" className="mt-3 inline-flex rounded-lg border border-stone-300 px-3 py-2 text-xs font-bold text-gold-deep">Buka gambar QRIS</a> : null}</div> : null}
