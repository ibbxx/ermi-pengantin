...
/**
 * Compress and upload payment proof image to Supabase Storage.
 * Restricts the uploaded file size to at most 500 KB by dynamically reducing quality and dimensions.
 *
 * @param file - The File object from input[type=file]
 * @returns The public URL of the uploaded payment proof
 */
export async function uploadPaymentProof(file: File): Promise<string> {
  const MAX_BYTES = 500 * 1024; // 512,000 bytes

  if (!file.type.startsWith('image/')) {
    throw new Error('Hanya mendukung file gambar.');
  }

  // Compress with default parameters (max 1200px, 0.82 quality)
  let maxDim = 1200;
  let quality = 0.82;
  let compressedBlob = await compressImage(file, maxDim, quality);

  // Dynamically reduce dimensions and quality if the compressed file still exceeds 500 KB
  if (compressedBlob.size > MAX_BYTES) {
    maxDim = 1000;
    quality = 0.70;
    compressedBlob = await compressImage(file, maxDim, quality);
  }
  ...
  // Generate safe and unique path in 'payment-proofs' folder
  const timestamp = Date.now();
  const safeName = file.name
    .replace(/[^a-zA-Z0-9._-]/g, '_')
    .toLowerCase();
  const filePath = `payment-proofs/${timestamp}-${safeName}`;
  ...
  if (error) {
    if (error.message?.includes('Bucket not found')) {
      throw new Error(
        `Bucket "${BUCKET_NAME}" tidak ditemukan di Supabase.\n\n` +
        `Silakan buat bucket baru dengan nama "${BUCKET_NAME}" di dashboard Supabase Anda (Storage > New bucket) dan pastikan mencentang pilihan "Public".`
      );
    }
    throw new Error(`Gagal mengunggah bukti transfer: ${error.message}`);
  }
